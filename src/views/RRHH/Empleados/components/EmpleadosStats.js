...
import { cilUser, cilCheckCircle, cilClock, cilChart } from '@coreui/icons'

const EmpleadosStats = ({ estadisticas }) => {
  const { total = 0, activos = 0, vacaciones = 0, promedioRendimiento = 0 } = estadisticas || {}
  ...
  const stats = [
    ...
    },
    {
      ...
      progressValue: total > 0 ? Math.round((activos / total) * 100) : 0,
    },
    {
      label: 'En Vacaciones',
      value: vacaciones,
      ...
      progressValue: total > 0 ? Math.round((vacaciones / total) * 100) : 0,
    },
    {
      label: 'Rendimiento',
      value: `${Math.round(promedioRendimiento)}%`,
      icon: cilChart,
      color: '#8b5cf6', // Purple
      bgGradient: 'linear-gradient(135deg, rgba(139, 92, 246, 0.1), rgba(139, 92, 246, 0.05))',
      progressValue: Math.round(promedioRendimiento) || 0,
    },
  ]
